...
spectraData(ms2_qtof_comb)$spectrum_type <- "merged_MSMS_all_energies"
...
counts_single_energy <- sd %>%
  filter(msLevel == 2, spectrum_type == "intersect_single_energy") %>%
...
```{r}
## Total MS2 spectra where spectrum_type == "merged_MSMS_all_energies"
n_merged_ms2 <- sd %>%
  filter(msLevel == 2, spectrum_type == "merged_MSMS_all_energies") %>%
  nrow()
