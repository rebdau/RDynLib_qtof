...
– A column named **`data_file`** containing the filenames of the `.mzXML` files.\
– A column named **`sample_name`** with the corresponding sample names.\
The sample name for blank control files should be `"blank"`.

The direcotry hierarchy is as follows: – `my.workdir/`\
– `my.workdir/sample_data/` containing the Excel file\
– `my.workdir/mzml/` containing the `.mzXML` files
...
sample_data <- read_xlsx(file.path(pth, "/sample_data/qtof_data_files.xlsx")) |>
    as.data.frame()
...
The Lockmass scans from Waters raw files can be filtered out by converting the files to `.mzXML` format using **MSConvert** with the `Waters DDA Processing` filter enabled.
