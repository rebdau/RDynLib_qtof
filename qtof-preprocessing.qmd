...
The sample/file information is retrieved from an Excel file *qtof_data_files.xlsx* that must meet the following minimal requirements:

– A column named *data_file* containing the filenames of the *.mzML* files. – A column named *sample_name* with the corresponding sample names. The sample name for blank control files should be `"blank"`. – A column named *sample_id*.

All files should be placed locally within the working directory containing also this file (i.e., *qtof-preprocessing.qmd*). The directory hierarchy is expected to be the following:

– *\<my.workdir\>/data/mzML*: directory containing all *.mzML* files. – *\<my.workdir\>/data/sample_data/*: directory containing the Excel file with the sample and file information.
...
The following code may help to create the table with data files and sample names:
...
To ensure we would not loose any signal from metabolites known to be present in the data set we next extract ion chromatograms for *m/z* ranges of known compounds.
...
The distribution of chromatographic peak widths in the retention time dimension ranged from approximately 1,70 to 87,29 seconds, with a median of about 12,83 seconds. In the m/z dimension, peak widths were much narrower, ranging from 0 to 0.1046 m/z units, with a median of around 0.0193.

We next perform the chromatographic peak refinement to reduce the number of potential *centWave*-specific peak detection artifacts.
...
Here we add *CleanPeaksParam* to remove chromatographic peaks with a retention time range larger than the provided maximal acceptable width (maxPeakwidth) based on (75th percentile of peak widths) \* 3.
...
qtof <- refineChromPeaks(qtof, cpp)
```
...
We observe that peak widths increased after refinement.
...
At last we calculate chromatographic peak quality metrics using the `chromPeakSummary()` function with the `BetaDistributionParam()`. This resulting extra peak shape descriptors, including *beta_cor* can then be used to filter low quality peaks. *beta_cor* is the correlation coefficient between the extracted ion chromatogram (EIC) peak and an idealized Gaussian peak fitted to it. We can use this peak shape descriptor to filter out junk peaks. Here, we first look at the distribution of beta_cor:

```{r}
beta_metrics <- chromPeakSummary(qtof, param = BetaDistributionParam())
...
`beta_cor` ranges from -1.0 to 1.0 and has median `r median(beta_metrics[, "beta_cor"], na.rm = TRUE)`. Most peaks thus have a `beta-cor`close to 1. Negative beta_cor values indicate anti-correlation of observed signal with Gaussian-like model. These peaks seem to have a valley instead of an apex, maybe multiple shoulders or just baseline fluctuation. Peaks with **negative beta_cor must be filtered out**. But we could first refine the peaks and evaluate the beta_cor values again after refinement before filtering.
...
cid <- rownames(beta_metrics)[which(beta_metrics[, "beta_cor"] < -0.5)] |>
    sample(size = 12)
eics <- chromPeakChromatograms(qtof, peaks = cid)
...
Now we want to filter out junk peaks based on `beta_cor < 0`. This will remove `r length(which(beta_metrics[, "beta_cor"] < 0))` peaks with a negative beta correlation and `r sum(is.na(beta_metrics[, "beta_cor"]))` peaks with a missing beta correlation value of the in total `r nrow(beta_metrics)` chromatographic peaks.

```{r}
#' Keep only chrom peaks with a positive correlation, removing also those
#' with a missing value.
qtof <- filterChromPeaks(qtof, keep = which(beta_metrics[, "beta_cor"] > 0))
```
...
As a final step we next perform gap-filling to reduce the number of missing values in the data set. Note that gap-filling would not be needed, as we don't use feature abundances in this analysis.
...
corrected_features <- featureDefinitions(qtof)
