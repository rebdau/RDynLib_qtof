...
Next we perform chromatographic peak detection in the extracted ion
chromatograms to determine the upper retention time limit for the retention time
filtering of the data set (using `filterRt()`).

```{r}
#' Identify chromatographic peaks in the data; use the same CentWaveParam
#' we use below for the full data set

#' We need to evaluate the chromPeaks()[, "rt"] column.
#' An easy solution to define the maximum rt to use in `filterRt()` below would
#' be to simply take the maximum rt of the identified chromatographic peaks.
#' Maybe combine that also with "maxo" column? i.e. consider only
#' chromatographic peaks with a maxo > 5000 (or another value)?
```
